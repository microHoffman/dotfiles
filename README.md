...
The SEO suite, hosted Notion MCP, and official Sentry plugin are disabled by the
base Codex config. Start `codex --profile own` to enable the OWN and Notion MCP
servers, `codex --profile seo` for the SEO suite, or `codex --profile sentry`
for Sentry. The OWN profile requires a separate explicit chat confirmation
before each Notion write. The Sentry plugin supplies its complete upstream
skill set and hosted MCP configuration without a dotfiles-maintained skill
wrapper. `grill-me`, `grill-with-docs`,
